...
function isUnsafeInteger(num) {
  ...
  return num >= Number.MIN_SAFE_INTEGER !== isSafeInteger;
}
...
function convertToExponential(num) {
  ...
  const number = num.toExponential();
  return number;
}
...
function convertToFixed(num, precision) {
  ...
  const number = num.toFixed(precision);
  return number;
}
...
function convertToPrecision(num, precision) {
  ...
  const number = num.toPrecision(precision);
  return number;
}
